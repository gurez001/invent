            ...
            render={({ field }) => (
                <div>
                    ...
                    <Select  onValueChange={field.onChange}
                        value={field.value}>
                        <SelectTrigger aria-labelledby={`label-${name}`} className={`w-full ${class_style}`}>
                            <SelectValue placeholder={placeholder} />
                        </SelectTrigger>
                        <SelectContent >
                            {drop_down_selector.map((item) => (
                    ...
                    {errorMessage && (
                        <p className="text-red-600 text-xs pt-[3px]">{errorMessage}</p>
                    )}
                </div>
            )}
        />
